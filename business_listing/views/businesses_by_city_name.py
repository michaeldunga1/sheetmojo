...
def businesses_by_city_name(request, country_code, city):
    query = request.GET.get('q', '').strip()
    country_name = country_info.name(country_code) or country_code
    businesses = BusinessDetails.objects.filter(country=country_code, city=city)
    if query:
    ...
    context = {
        ...
        'page_title': f"Businesses in {city}, {country_name}",
